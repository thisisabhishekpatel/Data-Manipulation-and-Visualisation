...
order_date_tbl %>%
    ...
    mutate(order_date_date = order_date_chr2 %>% ymd_hms())
...
bike_sales_y_tbl %>%
    mutate(cumulative_sales = cumsum(sales),
           cumulative_sales_pct = cumulative_sales / sum(sales) )%>%
...
bike_sales_m_tbl %>%
    ...
    mutate(roll_mean_6 = rollmean(sales, k = 6, na.pad = TRUE, aligh = "right", fill = NA))
